...
\begin{axis}[
    ...
    % 
    xtick={\getxmin,1.19,\getxmax},
    % xticklabels={},
    % 
    ymin=0,
    ymax=1,
    ytick={\getymin,0.6,\getymax},
    % 
    ...
    legend entries={$b=1-\theta-\frac{(1-\theta)h^2}{4}-\frac{h}{4}-\frac{a_2}{2p}$},
]
...
p=4,
a2=2,
-0.2125*x**2 - 0.25*x + 0.6 ;
} 
\addsymbol{0.45,text opacity=1}{$ \phi^* = \phi^e$}
\addpoint{0}{0}
\addpoint{1}{90}
;
...
\path node [align=left,]at(axis description cs:0.2,0.25) {$ \phi^* <\phi^e$   }
node[align=left,] at(axis description cs:0.82,0.25) {$ \phi^* > \phi^e$};
